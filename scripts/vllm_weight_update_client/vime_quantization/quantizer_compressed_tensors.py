...
try:
    from .int4_qat import fake_int4_quant_cuda
except ImportError:
...
def pack_layer(weight, group_size, sym=True):
    if fake_int4_quant_cuda is None:
        raise RuntimeError(
            "Vime's fake_int4_quant_cuda extension is not built. "
            "Run `python setup.py build_ext --inplace` under "
            "scripts/vllm_weight_update_client/vime_quantization/int4_qat "
            "in the client image."
        )
